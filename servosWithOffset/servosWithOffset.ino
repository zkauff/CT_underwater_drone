...
int pos = 0;    
int offSet = 5; //the degree offset on each of the servos
int i;
void setup() {
  Serial.begin(9600);
...
void loop() {
  for (pos = 0; pos <= 360; pos += 2) {
    ...
    servo1.write(val);
    servo4.write(val);//TODO: calculate mirrored servo
    val =  maxAngle * sin(pos * (2* M_PI ) / 360 + offSet) + maxAngle;
    servo2.write(val);
    servo5.write(val);//TODO: calculate mirrored servo
    val =  maxAngle * sin(pos * (2* M_PI ) / 360 + offSet * 2) + maxAngle;
    servo3.write(val);
    servo6.write(val); //TODO: calculate mirrored servo
    delay(10);             
  }
}
